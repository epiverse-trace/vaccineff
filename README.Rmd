...
*{{ packagename }}* provides a minimal cohort dataset that can be used to test out vaccineff package. 
...
The function `make_vaccineff_data` allows to define different aspects of the design from which the data is generated  (such as vaccination dates, inmunization delays and potential confounding factors), creating a vaccineff object that will be used to estimate VE with the `effectiveness` function using a Cox regression model.  The  proportional hazard assumption can also be plotted and tested by setting `type = "loglog"`.
