...
1. `make_vaccineff_data`: This function returns  the cohort data object with the relevant information on immunization for the study. This function also allows to create a matched cohort  to control for confounding variables. `make_vaccineff_data`  supports methods like `summary()` to check matching balance and the sizes of matched, excluded, and removed populations, and `get_dataset()` to extract the matched cohort.

2.  `plot_coverage`: This function returns a plot of the vaccine coverage or the cumulative coverage.
...
```
For details on the estimation of VE in cohort studies see *AQUI* 
